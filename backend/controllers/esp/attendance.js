...
const post = async (req, res) => {
    ...
    // --- Check if the student already have an attendance
...
const destroy = async (req, res) => {
    const { eveid, rfid } = req.params
    if (!rfid) return res.status(404).send("Student's rfid required.")
    if (!eveid) return res.status(400).send("Event id required.")
    ...
    // --- Check if the student have an attendance
    const attdata = { entryId: entry.id, studentId: student.id }
    const attendance = await models.attendance.Attendance.findOne({ where: attdata })
    if (!attendance) return res.status(404).send("Student doesn't have attendance to this event.")

    // --- Delete attendance
    await attendance.destroy()
    return res.send(`Attendance deleted successfully for ${student.name}.`)
}
